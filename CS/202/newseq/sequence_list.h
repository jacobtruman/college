...
//   bool is_pitem( ) const
//     Postcondition: A true return value indicates that there is a valid
//     "previous" item that may be retrieved by activating the previous
//     member function (listed below). A false return value indicates that
//     there is no valid previous item.
//
//   value_type current( ) const
//     Precondition: is_item( ) returns true.
//     Postcondition: The item returned is the current item in the sequence.
//
//   value_type previous( ) const
//     Precondition: is_pitem( ) returns true.
//     Postcondition: The item returned is the previous item in the sequence.
//
// VALUE SEMANTICS for the sequence class:
//    Assignments and the copy constructor may be used with sequence objects.

#ifndef MAIN_SKITCH_SEQUENCE_LIST_H
#define MAIN_SKITCH_SEQUENCE_LIST_H
...
namespace main_skitch_5
{
	...
	// CONSTANT MEMBER FUNCTIONS
		...
		bool is_pitem( ) const { return (precursor != NULL); }
		value_type current( ) const;
		value_type previous( ) const;
    private:
	node *head_ptr;
